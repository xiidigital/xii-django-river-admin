...
from xii.django_river.models import State, Workflow, TransitionMeta, Function, OnTransitHook
...
class TransitionHookViewTest(TestCase):
    ...
    def test__shouldCreateTransitionHook(self):
        ...
        user.user_permissions.add(Permission.objects.get(codename="add_ontransithook"))
    ...
    def test__shouldRejectCreateWithoutPermission(self):
        ...
        assert_that(OnTransitHook.objects.first(), is_(none()))
    ...
    def test__shouldReturnNotFoundWhenAnInexistentTransitionHookIsRequestedToDelete(self):
        ...
        user.user_permissions.add(Permission.objects.get(codename="delete_ontransithook"))
        self.client.force_login(user)
    ...
    def test__shouldRejectDeleteWithoutPermission(self):
        ...
        response = self.client.delete('/transition-hook/delete/%d/' % transition_hook.id)
        assert_that(response.status_code, equal_to(HTTP_403_FORBIDDEN))
        assert_that(OnTransitHook.objects.filter(pk=transition_hook.id).first(), not_none())
