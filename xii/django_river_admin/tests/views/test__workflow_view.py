...
from xii.django_river.models import State, Workflow, TransitionMeta, Transition

from xii.django_river_admin import DEFAULT_ADMIN_ICON
...
class WorkflowViewTest(TestCase):
    ...
    def test__shouldNotCreateWorkflowWhenFieldNameIsMissing(self):
        ...
        user.user_permissions.add(Permission.objects.get(codename="add_workflow"))
        self.client.force_login(user)
    ...
    def test__shouldRejectCreateWithoutPermission(self):
        ...
        assert_that(Workflow.objects.filter(field_name="test-field").first(), is_(none()))
    ...
    def test__shouldReturnNotFoundWhenAnInexistentWorkflowIsRequestedToDelete(self):
        ...
        user.user_permissions.add(Permission.objects.get(codename="delete_workflow"))
        self.client.force_login(user)
    ...
    def test__shouldRejectDeleteWithoutPermission(self):
        ...
        response = self.client.delete('/workflow/delete/%d/' % workflow.id)
        assert_that(response.status_code, equal_to(HTTP_403_FORBIDDEN))
        assert_that(Workflow.objects.filter(pk=workflow.id).first(), not_none())
