...
from xii.django_river.models import Function, OnApprovedHook, State, TransitionApprovalMeta, \
...
from xii.django_river.models.hook import AFTER, BEFORE
...
class WorkflowDto(serializers.ModelSerializer):
    ...
    def get_admin_name(self, obj):
        from xii.django_river_admin import site
...
class FunctionDto(serializers.ModelSerializer):
    created_by = UserDto(allow_null=True)
    updated_by = UserDto(allow_null=True)
    approved_by = UserDto(allow_null=True)

    class Meta:
        model = Function
        fields = [
            'id', 'name', 'body', 'version', 'date_created', 'date_updated',
            'created_by', 'updated_by', 'is_approved', 'approved_by', 'approved_at',
        ]
